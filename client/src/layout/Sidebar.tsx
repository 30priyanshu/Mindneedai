...
const userNavItems: NavItem[] = [
  ...
  { label: 'Mood Tracker', path: '/mood', icon: <Calendar size={24} /> },
  {
    label: 'Analysis',
    path: '/analyze',
    icon: <Activity size={24} />,
    children: [
      { label: 'Text Analysis', path: '/analyze/text', icon: <FileText size={20} /> },
      { label: 'Video Analysis', path: '/analyze/video', icon: <Video size={20} /> },
      { label: 'Audio Analysis', path: '/analyze/audio', icon: <Mic size={20} /> },
    ],
  },
  { label: 'Connect Doctor', path: '/connect-doctor', icon: <UserCheck size={24} /> },
  { label: 'Assessments', path: '/assessments', icon: <ClipboardList size={24} /> },
  { label: 'Wellness Forms', path: '/wellness-forms', icon: <FileText size={24} /> },
  { label: 'History', path: '/history', icon: <History size={24} /> },
  { label: 'Settings', path: '/settings', icon: <Settings size={24} /> },
  { label: 'Profile', path: '/profile', icon: <User size={24} /> },
  { label: 'Help', path: '/help', icon: <HelpCircle size={24} /> },
];

const doctorNavItems: NavItem[] = [
  { label: 'Dashboard', path: '/doctor/dashboard', icon: <LayoutDashboard size={24} /> },
  { label: 'Patients', path: '/doctor/patients', icon: <Users size={24} /> },
  { label: 'Wellness Forms', path: '/wellness-forms', icon: <FileText size={24} /> },
  { label: 'Create Wellness Form', path: '/doctor/create-wellness-form', icon: <Stethoscope size={24} /> },
  { label: 'Patient Assessments', path: '/doctor/patient-assessments', icon: <ClipboardList size={24} /> },
  { label: 'Profile', path: '/doctor/profile', icon: <User size={24} /> },
  { label: 'Settings', path: '/settings', icon: <Settings size={24} /> },
  { label: 'Help', path: '/help', icon: <HelpCircle size={24} /> },
];

const isPathActive = (path: string, locationPath: string): boolean => {
  if (path === '/dashboard' || path === '/doctor/dashboard') {
    return locationPath === '/' || locationPath === '/dashboard' || locationPath === '/doctor/dashboard';
  }
  if (path === '/analyze') return locationPath.startsWith('/analyze');
  if (path === '/assessments') return locationPath.startsWith('/assessments');
  if (path === '/wellness-forms') return locationPath.startsWith('/wellness-forms');
  if (path === '/doctor/patient-assessments') return locationPath.startsWith('/doctor/patient-assessments');
...
export const Sidebar: React.FC = () => {
  ...
  const navItems = useMemo(() => (role === 'doctor' ? doctorNavItems : userNavItems), [role]);
  ...
  return (
    <aside
      className={cn(
        ...
        isMobile && (collapsed ? 'w-0 overflow-hidden' : 'w-[18rem] shadow-2xl')
      )}
      style={{ zIndex: 40 }}
    >
      ...
      <div className="h-24 flex items-center justify-between px-4 sm:px-6 border-b border-slate-200 dark:border-slate-800/50 bg-slate-50 dark:bg-slate-900/50 flex-shrink-0 relative group">
        <Link to="/" className={cn("flex items-center gap-3 transition-transform hover:scale-[1.02]", collapsed ? "mx-auto" : "flex-1 overflow-hidden")}>
        ...
        {!isMobile && (
          ...
          >
            <PanelLeftClose size={14} strokeWidth={2.5} className="ml-[1px]" />
          </button>
        )}
      </div>
                    ...
                    >
                      ...
                      {!collapsed && (
                        <>
                          ...
                          <ChevronRight
                            size={20}
                            className={cn('transition-transform flex-shrink-0', analysisExpanded && 'rotate-90')}
                          />
                        </>
                      )}
                    </button>
