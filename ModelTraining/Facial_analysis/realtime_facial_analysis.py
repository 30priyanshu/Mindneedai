...
class Bottleneck(nn.Module):
    expansion = 4
    def __init__(self, in_channels, out_channels, i_downsample=None, stride=1, dropout_rate=0.0):
...
def ResNet50(num_classes, channels=3, dropout_rate=0.05):
    return ResNet(Bottleneck, [3,4,6,3], num_classes, channels, dropout_rate=dropout_rate)
...
class RealtimeFacialAnalyzer:
    def __init__(self, model_path, device='cuda', lstm_model_path=None, use_temporal_smoothing=True):
        ...
        self.model = ResNet50(num_classes=7, channels=3, dropout_rate=0.0)
...
def main():
    ...
    model_path = script_dir / "FER_static_ResNet50_optimized.pt"
    lstm_model_path = script_dir / "FER_dinamic_LSTM_Aff-Wild2.pt"
    
    if not model_path.exists():
        print(f"Error: Model not found at {model_path}")
        print(f"Please place the trained model file 'FER_static_ResNet50_optimized.pt' in: {script_dir}")
        sys.exit(1)
