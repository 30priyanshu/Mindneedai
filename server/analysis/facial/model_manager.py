...
class FacialModelManager(BaseModelManager):
    """
    Facial Modality Model Manager.
    Responsibility: Manage PyTorch model versioning for facial models with JSON + DB registry.
    """

    def __init__(self, cache_dir: str = "./model_cache/facial_analysis", initial_version: str = "v1"):
    ...
    def _sync_db(self, version_id: str, model_path: str, is_active: bool, fine_tuned: bool) -> None:
        try:
            with SessionLocal() as db:
                ...
                if is_active:
                    repo.set_active_version(version_id, "fac_")
        except Exception as e:
    ...
    def activate_version(self, version_id: str) -> None:
        if not version_id or version_id not in self.versions:
            raise ValueError("Version ID not found in facial registry.")
        logger.info(f"Activating new facial model version: {version_id}")
    ...
    def rollback(self) -> None:
        if not self._previous_version:
            raise RuntimeError("No previous facial model version to rollback to.")
