...
class TextModelManager(BaseModelManager):
    """
    Text Modality Model Manager.
    Responsibility: Handle HuggingFace model versioning for text models with JSON + DB registry.
    """

    def __init__(self, cache_dir: str = "./model_cache/text_analysis", initial_version: str = "cardiffnlp/twitter-roberta-base-sentiment-latest"):
    ...
    def _sync_db(self, version_id: str, model_path: str, is_active: bool, fine_tuned: bool) -> None:
        try:
            with SessionLocal() as db:
                ...
                if is_active:
                    repo.set_active_version(version_id, "txt_")
        except Exception as e:
    ...
    def activate_version(self, version_id: str) -> None:
        if not version_id or version_id not in self.versions:
            raise ValueError("Version ID not found in text registry.")
        logger.info(f"Activating new text model version: {version_id}")
    ...
    def rollback(self) -> None:
        if not self._previous_version:
            raise RuntimeError("No previous text model version to rollback to.")
